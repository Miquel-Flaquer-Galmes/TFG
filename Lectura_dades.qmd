...
library(tidyverse)
library(jsonlite)
Enzymes_KO <- jsonlite::fromJSON("Dades_Pere/enzymes_KO.json")
Enzymes_lite <- jsonlite::fromJSON("Dades_Pere/enzymes_lite.json")
Reaction_lite_pw <- jsonlite::fromJSON("Dades_Pere/reactions_lite_pw.json")
```
...
## ── 0. CONFIGURATION ─────────────────────────────────────────
ABUND_FILE   <- "MTG_abund.tsv" # definim la ruta de la taula d'abundàncies
...
ec_to_rxns <- lapply(ec_rxn_raw, function(ec_entry) {
  unique(unlist(ec_entry[["reaction"]]))  # field name per your JSON structure
})
names(ec_to_rxns) <- sapply(ec_rxn_raw, `[[`, "enzyme")  # adjust key if needed
